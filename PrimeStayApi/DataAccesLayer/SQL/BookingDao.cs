        ...
        public int Create(BookingEntity model)
        {
            ...
            using (IDbConnection connection = DataContext.Open())
            {
                ...
                var avaliableRooms = connection.QueryFirst("SELECT count(*) as Num_of_bookings, (SELECT num_of_avaliable FROM room WHERE id=@room_id)" +
                                                           " as Number_of_avail_Rooms FROM booking WHERE room_id = @room_id start_date BETWEEN @start_date AND @end_date" +
                                                                                                       " AND end_date BETWEEN @start_date AND @end_date", new { model.Room_id, model.Start_date, model.End_date });
